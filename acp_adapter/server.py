...
class HermesACPAgent(SlashCommandsMixin, acp.Agent):
    ...
    def _wire_turn_callbacks(
        ...
        # Some ACP clients only render the final message.  Let their launcher opt
        # out of deltas without affecting normal ACP streaming clients.
        agent.stream_delta_callback = (
            None if os.environ.get("HERMES_ACP_DISABLE_STREAMING", "").strip() == "1"
            else cbs.stream_delta_cb
        )
        return cbs
