...
def _count_commits_behind(
    ...
    *,
    ...
    ref = f"{remote}/{branch}"
    try:
        # Limit the fetch to the branch used by the behind-count. This avoids
        # transferring every remote head, which is especially expensive for
        # the large upstream repository.
    ...
    if is_shallow:
        # No history to count across the shallow boundary. `remote/main` may not
        # be a tracking ref in a `clone --depth 1`, so prefer FETCH_HEAD (just
        # updated by the fetch above) and fall back to remote/main.
        head_rev = _git_stdout(["rev-parse", "HEAD"], cwd=repo_dir)
        target_rev = (
            _git_stdout(["rev-parse", "FETCH_HEAD"], cwd=repo_dir)
            or _git_stdout(["rev-parse", ref], cwd=repo_dir)
        )
...
def _check_via_local_git(repo_dir: Path) -> Optional[int]:
    ...
    installs where origin is the official repo. If origin is the official repo
    over SSH, use an HTTPS ls-remote probe instead of passive SSH auth prompts.
    """
    ...
    # clone the history stops at a single commit, so preserve the boundary and
    # compare tip SHAs instead of reporting a bogus large behind count.
    shallow = _git_stdout(["rev-parse", "--is-shallow-repository"], cwd=repo_dir)
    is_shallow = shallow == "true"

    for remote in remotes:
        behind = _count_commits_behind(repo_dir, remote, is_shallow=is_shallow)
...
def check_for_updates() -> Optional[int]:
    ...
    if embedded_rev:
        ...
        if not (repo_dir / ".git").exists():
            # No git checkout: compare the installed package version with PyPI.
...
def _compute_git_banner_state(repo_dir: Optional[Path] = None) -> Optional[dict]:
    ...
    upstream_ref = "upstream/main" if _git_remote_exists(repo_dir, "upstream") else "origin/main"
