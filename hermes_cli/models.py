...
def probe_api_models(
    ...
    # Codex exposes its catalog under a non-OpenAI response shape and requires
    # the client_version query parameter.  Keep this before the generic /models
    # probing loop.
    codex_root = "https://chatgpt.com/backend-api/codex"
    if normalized in {codex_root, f"{codex_root}/v1"}:
        url = f"{codex_root}/models?{urllib.parse.urlencode({'client_version': '1.0.0'})}"
        ...
        try:
            ...
            for item in entries:
                ...
                if (slug and item.get("supported_in_api") is not False
                        and str(item.get("visibility") or "").strip().lower() not in {"hide", "hidden"}
                        and slug not in models_out):
                    models_out.append(slug)
            return _probe_result(models_out, url, codex_root)
        except Exception:
            return _probe_result(None, url, codex_root)
