...
_GROUP_CHAT_ENV = {
    ...
    # Signal group IDs are historically configured through this name.
    ...
    def _chat_scoped_grant(self, source, adapter_profile, is_group: bool, allow_adapter_delegation: bool) -> bool:
        ...
        if is_group and source.chat_id:
            ...
            if chat_allowlist_env:
                allowed = _coerce_allow_set(_platform_gate_env(chat_allowlist_env))
                ids = {source.chat_id}
                if source.platform == Platform.SIGNAL and source.chat_id.startswith("group:"):
                    ids.add(source.chat_id.split("group:", 1)[1])
                if "*" in allowed or bool(ids & allowed):
                    return True
